...
export class ReactiveEffect {
    ...
    // fn：用户编写的函数
    // 如果fn中依赖的数据发生变化后，需要重新调用scheduler，即调用run()
    ...
    run() {
        ...
        // 让fn执行
        if (!this.active) {
            return this.fn(); // 不是激活（响应式）的，执行后，什么都不用做
        }
        let lastEffect = activeEffect;
        try {
            activeEffect = this; // 绑定到全局变量
            preCleanEffect(this); // effect重新执行前，需要将上一次的依赖情况effect.deps清空
            this._running++;
            return this.fn(); // 依赖收集
        } finally {
            this._running--;
            postCleanEffect(this);
            activeEffect = lastEffect; // 不能直接将activeEffect设置为undefined，因为effect可能嵌套
        }
    }

    // 停止所有的effect，不参加响应式处理
    stop() {
        if (this.active) {
            this.active = false;
            preCleanEffect(this);
...
export function trackEffect(effect, dep) {
    ...
    if (dep.get(effect) !== effect._trackId) {
        ...
        if (oldDep !== dep) {
            if (oldDep) {
                cleanDepEffect(oldDep, effect); // 删除掉老的
            }
            effect.deps[effect._depsLength++] = dep; // 换成新的，永远按照本次最新的来存放
        } else {
            effect._depsLength++;
        }
    }
...
export function triggerEffects(dep) {
    for (const effect of dep.keys()) {
        ...
        if (!effect._running) {
            if (effect.scheduler) {
                effect.scheduler(); // 如果不是正在执行，才能执行 -> effect.run()
            }
        }
    }
}
