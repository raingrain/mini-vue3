...
export function createRenderer(renderOptions) {
    ...
    const mountChildren = (children, container, anchor, parentComponent) => {
        normalize(children);
        for (let i = 0; i < children.length; i++) {
            patch(null, children[i], container, anchor, parentComponent); // children[i]可能是纯文本元素
        }
    };

    const mountElement = (vnode, container, anchor, parentComponent) => {
        const {type, children, props, shapeFlag, transition} = vnode;
        // 第一次渲染的时候让虚拟节点和真实的dom创建关联 vnode.el = 真实dom
    ...
    const processElement = (n1, n2, container, anchor, parentComponent) => {
        if (n1 === null) {
            mountElement(n2, container, anchor, parentComponent); // 初始化操作
        } else {
    ...
    const patchProps = (oldProps, newProps, el) => {
        ...
        for (let key in oldProps) {
            if (!(key in newProps)) {
                hostPatchProp(el, key, oldProps[key], null); // 以前多的现在没有了，需要删除掉
            }
        }
    };
        ...
        if (i > e1) {
            ...
            for (let i = s1; i <= e1; i++) {
                ...
                if (newIndex == undefined) {
                    unmount(vnode, parentComponent); // 如果新的里面找不到则说明老的有的要删除掉
                } else {
                    // 比较前后节点的差异，更新属性和儿子
                    // 我们i可能是0的情况，为了保证0是没有比对过的元素，直接i+1
                    newIndexToOldMapIndex[newIndex - s2] = i + 1; // [5,3,4,0]
                    patch(vnode, c2[newIndex], el); // 服用
                }
            }
            // 调整顺序
            // 我们可以按照新的队列，倒序插入insertBefore通过参照物往前面插入
    ...
    const patchElement = (n1, n2, container, anchor, parentComponent) => {
        ...
        const {patchFlag, dynamicChildren} = n2; // 在比较元素的时候 针对某个熟悉来去比较
        if (patchFlag) {
            ...
            patchProps(oldProps, newProps, el); // hostPatchProp 只针对某一个属性来处理  class style event attr
        }
        if (dynamicChildren) {
            patchBlockChildren(n1, n2, el, anchor, parentComponent); // 线性比对
        } else {
            patchChildren(n1, n2, el, anchor, parentComponent); // 全量diff
        }
    };
    ...
    const updateComponentPreRender = (instance, next) => {
        ...
        Object.assign(instance.slots, next.children); // 组件更新的时候 需要更新插槽
    };

    function renderComponent(instance) {
        const {render, vnode, proxy, props, attrs, slots} = instance; // attrs, props = 属性
        if (vnode.shapeFlag & ShapeFlags.STATEFUL_COMPONENT) {
            return render.call(proxy, proxy);
        } else {
            return vnode.type(attrs, {slots}); // 此写法不用使用了，vue3中没有任何性能优化，函数式组件
        }
    }
    ...
    function setupRenderEffect(instance, container, anchor, parentComponent) {
        const componentUpdateFn = () => {
            ...
            if (!instance.isMounted) {
                ...
                if (next) {
                    updateComponentPreRender(instance, next); // 更新属性和插槽
                }
                if (bu) {
                    invokeArray(bu);
                }
        ...
        if (isKeepAlive(vnode)) {
            instance.ctx.renderer = {
                ...
                move(vnode, container, anchor) {
                    hostInsert(vnode.component.subTree.el, container, anchor); // 需要把之前渲染的dom放入到容器中
                },
    ...
    const updataProps = (instance, prevProps, nextProps) => {
        if (hasPropsChange(prevProps, nextProps)) {
            ...
            for (let key in nextProps) {
                instance.props[key] = nextProps[key]; // 用新的覆盖掉所有老的，更新
            }
    ...
    const shouldComponentUpdate = (n1, n2) => {
        ...
        if (prevChildren || nextChildren) {
            return true; // 有插槽直接走重新渲染即可
        }
        if (prevProps === nextProps) {
            return false;
        }
        return hasPropsChange(prevProps, nextProps || {}); // 如果属性不一致实则更新
    };
    ...
    const processComponent = (n1, n2, container, anchor, parentComponent) => {
        if (n1 === null) {
            if (n2.shapeFlag & ShapeFlags.COMPONENT_KEPT_ALIVE) {
                parentComponent.ctx.activate(n2, container, anchor); // 需要走keepAlive中的激活方法
            } else {
                mountComponent(n2, container, anchor, parentComponent);
            }
        } else {
            updateComponent(n1, n2); // 组件的更新
        }
    };

    const patch = (n1, n2, container, anchor = null, parentComponent = null) => {
        if (n1 == n2) {
            return; // 两次渲染同一个元素直接跳过即可
        }
        ...
        switch (type) {
            ...
            default:
                if (shapeFlag & ShapeFlags.ELEMENT) {
                    ...
                    processComponent(n1, n2, container, anchor, parentComponent); // 对组件的处理，vue3中函数式组件，已经废弃了，没有性能节约
                }
        }
        if (ref !== null) {
            setRef(ref, n2); // n2 是dom 还是 组件 还是组件有expose
        }
    };
    ...
    const unmount = (vnode, parentComponent) => {
        ...
        if (shapeFlag & ShapeFlags.COMPONENT_SHOULD_KEEP_ALIVE) {
            parentComponent.ctx.deactivate(vnode); // 需要找keep走失活逻辑
